...
//			_, err := zitadel.GetOrgIdpGithubEs(ctx, &zitadel.LookupOrgIdpGithubEsArgs{
...
// Enable if users should get prompted to link an existing ZITADEL user to an external account if the selected attribute matches, supported values: AUTO*LINKING*OPTION*UNSPECIFIED, AUTO*LINKING*OPTION*USERNAME, AUTO*LINKING*OPTION_EMAIL
func (o LookupOrgIdpGithubEsResultOutput) AutoLinking() pulumi.StringOutput {
	return o.ApplyT(func(v LookupOrgIdpGithubEsResult) string { return v.AutoLinking }).(pulumi.StringOutput)
}
